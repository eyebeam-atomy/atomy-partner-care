...
export default function Dashboard() {
  ...
  const [consultationData, setConsultationData] = useState({ date: today, content: '', next_meeting_date: nextWeek });
  ...
  const checkCrmAlertsAndStats = async (customersData: any[]) => {
    ...
    customersData.forEach(customer => {
      ...
      if (alertReasons.length > 0) {
        alerts.push({ ...customer, reasons: alertReasons, priority: isHighPriority ? 5 : 1 });
      }
    });
  ...
  const openCustomerModal = (customer: any) => {
    ...
    setConsultationData({ date: today, content: '', next_meeting_date: nextWeek });
      ...
      <div className="w-full max-w-md mx-auto bg-slate-50 min-h-screen sm:min-h-[850px] sm:rounded-[2.5rem] shadow-2xl overflow-hidden relative flex flex-col">
        <header className="px-6 pt-10 pb-6 bg-blue-600 text-white shadow-md z-10">
          <div className="flex justify-between items-center">
            <div><h1 className="text-2xl font-extrabold tracking-tight">{currentUser?.user_name}님,&nbsp;<span className="text-blue-200 font-medium text-lg">환영합니다! 👋</span></h1></div>
      ...
      {selectedCustomer && (
        <div className="fixed inset-0 bg-black/70 flex items-end sm:items-center justify-center p-0 sm:p-4 z-40">
          <div ref={modalScrollRef} className="bg-white w-full max-w-md h-[92vh] sm:h-[800px] overflow-y-auto rounded-t-[2rem] sm:rounded-[2.5rem] p-5 shadow-2xl relative scroll-smooth flex flex-col">
            <div className="sticky top-0 z-20 bg-white -mx-5 px-5 -mt-5 pt-5 pb-3 border-b border-slate-100 mb-4 shrink-0">
              <div className="w-12 h-1.5 bg-slate-200 rounded-full mx-auto mb-4 shrink-0 sm:hidden"></div>
              <div className="flex justify-between items-start">
                <div><h2 className="text-2xl font-bold text-slate-900">{selectedCustomer.name}</h2><div className="flex items-center gap-3 mt-1.5"><p className="text-slate-500 font-medium">{selectedCustomer.phone}</p><div className="flex gap-1.5"><a href={`tel:${selectedCustomer.phone}`} className="w-7 h-7 flex items-center justify-center bg-green-50 text-green-600 rounded-full border border-green-100 shadow-sm text-sm">📞</a><a href={`sms:${selectedCustomer.phone}`} className="w-7 h-7 flex items-center justify-center bg-blue-50 text-blue-600 rounded-full border border-blue-100 shadow-sm text-sm">✉️</a></div></div></div>
            ...
            <div className="mb-6 shrink-0 space-y-3">
              <form onSubmit={handleConsultationSubmit} className="bg-blue-50 p-4 rounded-2xl border border-blue-100 space-y-3">
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex gap-2">
                    <div className="flex-1 space-y-1"><label className="text-[10px] font-bold text-blue-600 ml-1">상담일</label><input type="date" required className="w-full px-3 py-3 bg-white border border-blue-200 rounded-xl text-sm outline-none text-slate-900 font-medium" value={consultationData.date} onChange={e => setConsultationData({...consultationData, date: e.target.value})} /></div>
                    <div className="flex-1 space-y-1"><label className="text-[10px] font-bold text-blue-600 ml-1">다음 미팅일</label><input type="date" className="w-full px-3 py-3 bg-white border border-blue-200 rounded-xl text-sm outline-none text-slate-900 font-medium" value={consultationData.next_meeting_date} onChange={e => setConsultationData({...consultationData, next_meeting_date: e.target.value})} /></div>
                  </div>
                  <input type="text" placeholder="상담 내용 입력" required className="w-full px-3 py-3 bg-white border border-blue-200 rounded-xl text-sm outline-none text-slate-900 placeholder:text-slate-500 font-medium" value={consultationData.content} onChange={e => setConsultationData({...consultationData, content: e.target.value})} />
                </div>
                <div className="flex gap-2 pt-1">{editingConsultationId && <button type="button" onClick={() => {setEditingConsultationId(null); setConsultationData({date: today, content: '', next_meeting_date: nextWeek});}} className="flex-1 py-3 bg-white border border-slate-300 rounded-xl text-sm font-bold text-slate-700">취소</button>}<button type="submit" className="flex-1 py-3 bg-blue-600 text-white rounded-xl text-sm font-bold shadow-sm">{editingConsultationId ? '수정 완료' : '등록하기'}</button></div>
              </form>
              <form onSubmit={handlePurchaseSubmit} className="bg-emerald-50 p-4 rounded-2xl border border-emerald-100 space-y-3">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {/* 🚀 [최종 해결!] 복잡한 로직 다 걷어내고, 리액트의 표준 상태 업데이트 방식만 사용합니다. */}
                  <input
                    list="product-list"
                    type="text"
                    placeholder="제품명 검색"
                    required
                    autoComplete="off"
                    className="w-full px-3 py-3 bg-white border border-emerald-200 rounded-xl text-sm outline-none text-slate-900 placeholder:text-slate-500 font-medium"
                    value={purchaseData.product_name}
                    onChange={(e) => setPurchaseData({...purchaseData, product_name: e.target.value})}
                  />
                  <datalist id="product-list">
                    {purchaseData.product_name.length > 0 && productList
                      .filter(p => p.name.includes(purchaseData.product_name))
            ...
            <div className="space-y-6 pb-10">
              <div className="space-y-2"><h4 className="font-bold text-slate-700 text-sm flex items-center gap-1"><span className="text-blue-500">💬</span> 지난 상담</h4>{history.filter(h => h.type === 'consultation').length === 0 ? (<p className="text-slate-400 text-center py-4 text-xs bg-slate-50 rounded-xl border border-slate-200">상담 내역 없음</p>) : (history.filter(h => h.type === 'consultation').map((item) => (
                <div key={`cons-${item.id}`} className="p-4 bg-white rounded-xl border border-slate-200 shadow-sm"><div className="flex justify-between items-center mb-2"><span className="text-xs font-bold text-blue-700 bg-blue-50 px-2.5 py-1 rounded-lg">{new Date(item.created_at).toLocaleDateString('ko-KR')}</span><div className="flex gap-2"><button onClick={() => editConsultation(item)} className="text-xs text-slate-500 font-bold">수정</button><button onClick={() => deleteConsultation(item.id)} className="text-xs text-slate-500 font-bold">삭제</button></div></div><p className="text-slate-800 text-sm whitespace-pre-wrap leading-relaxed">{item.content}</p>{item.next_meeting_date && (<p className="mt-3 text-xs font-bold text-blue-600 bg-blue-50 inline-block px-2.5 py-1.5 rounded-lg">🗓️ 다음 미팅: {new Date(item.next_meeting_date).toLocaleDateString('ko-KR')}</p>)}</div>
              )))}</div>
              <div className="space-y-2"><h4 className="font-bold text-slate-700 text-sm flex items-center gap-1"><span className="text-emerald-500">🛒</span> 지난 구매</h4>{history.filter(h => h.type === 'purchase').length === 0 ? (<p className="text-slate-400 text-center py-4 text-xs bg-slate-50 rounded-xl border border-slate-200">구매 내역 없음</p>) : (history.filter(h => h.type === 'purchase').map((item) => (
                  <div key={`pur-${item.id}`} className="p-4 bg-white rounded-xl border border-slate-200 shadow-sm"><div className="flex justify-between items-center mb-2"><span className="text-xs font-bold text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-lg">{new Date(item.created_at).toLocaleDateString('ko-KR')}</span><div className="flex gap-2"><button onClick={() => editPurchase(item)} className="text-xs text-slate-500 font-bold">수정</button><button onClick={() => deletePurchase(item.id)} className="text-xs text-slate-500 font-bold">삭제</button></div></div><div className="flex justify-between items-end gap-3 flex-nowrap"><div className="flex-1 min-w-0"><p className="text-slate-900 font-bold text-base truncate">{item.product_name}</p><p className="text-xs text-slate-500 mt-1">종료 예정: {new Date(item.expiry_date).toLocaleDateString('ko-KR')}</p></div><span className="shrink-0 text-sm bg-slate-100 text-slate-700 px-3 py-1.5 rounded-lg font-bold whitespace-nowrap self-end mb-1">{item.duration_days}일분</span></div></div>
              )))}</div>
            </div>
          </div>
        </div>
