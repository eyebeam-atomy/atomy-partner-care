...
export default function Dashboard() {
  ...
  const validateDateOnBlur = (value: string, label: string) => {
    if (!value) return true;
    if (!/^\d{4}-\d{2}-\d{2}$/.test(value)) {
      showAlert("날짜 입력 확인", `${label}은 20260707처럼 숫자 8자리로 입력해주세요.`);
      return false;
    }
    if (!isValidDateValue(value)) {
      showAlert("날짜 입력 확인", `${label}이 올바른 날짜가 아닙니다.`);
      return false;
    }
    return true;
  };
  ...
  const [hasReadTodayNotifications, setHasReadTodayNotifications] =
    useState(false);
  ...
  const handleChangePassword = async () => {
    if (newPwd.length < 8)
      return showAlert("입력 오류", "비밀번호는 8자리 이상이어야 합니다.");
    if (newPwd !== confirmPwd)
      return showAlert(
        "일치 확인",
        "비밀번호와 확인 칸의 입력이 서로 다릅니다.",
      );
  ...
  const handleSaveNotificationSettings = async () => {
    if (!currentUser?.id)
      return showAlert("저장 실패", "로그인 정보를 확인할 수 없습니다.");
  ...
  const checkCrmAlertsAndStats = async (customersData: any[]) => {
    ...
    const { alerts, monthlyStats } = await buildCrmAlertsAndStats(
      supabase,
      customersData,
    );
  ...
  const fetchCustomers = async () => {
    ...
    if (data) {
      const decryptedData = data.map((c) => ({
        ...
        birthday_type: c.birthday_type || "solar",
        concerns: Array.isArray(c.concerns) ? c.concerns : [],
        concern_other: c.concern_other || "",
      }));
      setCustomers(decryptedData);
      checkCrmAlertsAndStats(decryptedData);
      fetchReceivableSummary(decryptedData);
    }
  };

  const fetchReceivableSummary = async (customersData: any[]) => {
    if (!customersData.length) {
      setReceivableSummary({ totalAmount: 0, count: 0, items: [] });
      return;
    }
    ...
    const items = (data || []).map((item) => {
      ...
      return {
        ...item,
        customer_name: customer?.name || "이름 없음",
  ...
  const markPurchaseAsPaid = async (purchaseId: number) => {
    const { error } = await supabase
      .from("purchases")
      .update({ payment_status: "paid", paid_at: new Date().toISOString() })
      .eq("id", purchaseId);

    if (error) return showAlert("처리 실패", error.message);
    showAlert("입금 완료", "입금완료로 변경되었습니다.");
    fetchCustomers();
    fetchHistory();
  };

  const confirmMarkPurchaseAsPaid = (purchaseId: number) => {
    showConfirm(
      "입금완료 처리",
      "입금완료로 변경하시겠습니까?",
  ...
  const receivableGroups = receivableSummary.items.reduce(
    (groups: any[], item: any) => {
      const groupKey = item.customer_id;
      let group = groups.find((g) => g.customer_id === groupKey);
      ...
      group.totalAmount += item.price || 0;
  ...
  const closeReceivableDetailsIfEmpty = () => {
    if (receivableSummary.count === 0) setShowReceivableDetails(false);
  };

  const hasAdvancedSearch =
    advancedSearch.name.trim() !== "" ||
    advancedSearch.address.trim() !== "" ||
    advancedSearch.product.trim() !== "" ||
    advancedSearch.consultation.trim() !== "" ||
    advancedSearch.concern.trim() !== "";

  useEffect(() => {
    const fetchSearchResults = async () => {
      const productKeyword =
        isAdvancedSearchOpen && hasAdvancedSearch
          ? advancedSearch.product.trim()
          : searchType === "product"
            ? searchKeyword.trim()
            : "";
      const consultationKeyword =
        isAdvancedSearchOpen && hasAdvancedSearch
          ? advancedSearch.consultation.trim()
      ...
      if (!productKeyword) {
        ...
        const { data } = await supabase
          .from("purchases")
          .select("customer_id")
          .ilike("product_name", `%${productKeyword}%`);
        if (data)
          setPurchasedCustomerIds(
            Array.from(new Set(data.map((p: any) => p.customer_id))),
          );
      }
      ...
      if (!consultationKeyword) {
        ...
        if (data) {
          const matchedIds = data
            .filter((item) =>
              decrypt(item.content).includes(consultationKeyword),
            )
  ...
  const filteredCustomers = customers.filter((c) => {
    if (isAdvancedSearchOpen && hasAdvancedSearch) {
      ...
      const addressKeyword = advancedSearch.address.trim();
      const productKeyword = advancedSearch.product.trim();
      const consultationKeyword = advancedSearch.consultation.trim();
      const concernKeyword = advancedSearch.concern.trim();

      if (nameKeyword && !c.name.includes(nameKeyword)) return false;
      if (addressKeyword && !(c.address || "").includes(addressKeyword))
        return false;
      if (productKeyword && !purchasedCustomerIds.includes(c.id)) return false;
      if (consultationKeyword && !consultedCustomerIds.includes(c.id))
        return false;
      if (
        concernKeyword &&
        !(
          (c.concerns || []).includes(concernKeyword) ||
          (c.concern_other || "").includes(concernKeyword)
        )
      )
        return false;
      return true;
    ...
    if (searchType === "address")
      return (c.address || "").includes(searchKeyword.trim());
    if (searchType === "concern")
      return (
        (c.concerns || []).some((concern: string) =>
          concern.includes(searchKeyword.trim()),
        ) || (c.concern_other || "").includes(searchKeyword.trim())
      );
    if (searchType === "product") return purchasedCustomerIds.includes(c.id);
    if (searchType === "consultation")
      return consultedCustomerIds.includes(c.id);
    return true;
  });
  ...
  const openEditCustomer = (e: React.MouseEvent, customer: any) => {
    ...
    setFormData({
      ...
      birthday_type: customer.birthday_type || "solar",
      concerns: customer.concerns || [],
      concern_other: customer.concern_other || "",
    });
    setIsModalOpen(true);
  };

  const toggleCustomerConcern = (concern: string) => {
    setFormData((prev) => {
      const hasConcern = prev.concerns.includes(concern);
      const nextConcerns = hasConcern
      ...
      return {
        ...
        concern_other:
          concern === "기타" && hasConcern ? "" : prev.concern_other,
      };
    });
  };
  ...
  const handleCustomerSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!validateDateOnBlur(formData.birthday, "생일")) return;
    const payload = {
      ...
      birthday: formData.birthday ? encrypt(formData.birthday) : "",
      birthday_type: formData.birthday_type,
      concerns: formData.concerns,
      concern_other: formData.concern_other.trim(),
    };
    if (editingCustomerId)
      ...
      await supabase
        .from("customers")
        .insert([{ ...payload, creator_id: currentUser.id }]);
  ...
  const handleConsultationSubmit = async (e?: React.FormEvent) => {
    ...
    if (!consultationData.content.trim())
      return showAlert("내용 입력", "상담 내용을 입력해주세요.");
    const payload = {
      customer_id: selectedCustomer.id,
      content: encrypt(consultationData.content),
      created_at: new Date(consultationData.date).toISOString(),
      next_meeting_date: consultationData.next_meeting_date || null,
    };
  ...
  const deleteConsultation = (id: number) => {
    showConfirm("삭제", "상담 이력을 삭제하시겠습니까?", async () => {
  ...
  const handlePurchaseSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!validateDateOnBlur(purchaseData.date, "구매일")) return;
    const pDate = new Date(purchaseData.date);
    const productName = purchaseData.product_name.trim();
  ...
  const editPurchase = (item: any) => {
    ...
    setPurchaseData({
      date: new Date(item.created_at).toISOString().split("T")[0],
      duration: item.duration_days ? item.duration_days.toString() : "",
      custom_duration: item.duration_days ? item.duration_days.toString() : "",
  ...
  const deletePurchase = (id: number) => {
    showConfirm("삭제", "구매 이력을 삭제하시겠습니까?", async () => {
  ...
  return (
    <div className="min-h-screen bg-slate-100 sm:py-8 font-sans overflow-x-hidden">
      <div className="w-full max-w-md mx-auto bg-slate-50 min-h-screen sm:min-h-[850px] sm:rounded-[2.5rem] shadow-2xl overflow-hidden relative flex flex-col">
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              {currentUser?.user_id === "01093693777" && (
                <button
                  ...
                  className="whitespace-nowrap px-3 py-2 bg-amber-500 hover:bg-amber-400 rounded-xl text-white font-bold text-[11px] transition-colors shadow-lg animate-pulse flex items-center shrink-0"
                >
                  🛠️ 전체 관리
                </button>
              )}
            ...
            <div className="flex items-center justify-between bg-blue-700/30 p-2 rounded-2xl border border-blue-500/30">
              ...
              <div className="flex-1 flex justify-start">
                <button
                  ...
                  className="whitespace-nowrap px-3 py-2 bg-blue-500/50 hover:bg-blue-500 rounded-xl text-white font-bold text-xs transition-colors flex items-center gap-1"
                >
                  🔒 암호변경
                </button>
              </div>
              ...
              <div className="flex-1 flex justify-end">
                <button
                  ...
                  className="whitespace-nowrap px-4 py-2 bg-white text-blue-600 rounded-xl font-extrabold text-xs shadow-sm hover:bg-blue-50 transition-colors"
                >
                  로그아웃
                </button>
              </div>
          ...
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-6 z-[110]">
            <div className="bg-white w-full max-w-xs rounded-3xl p-6 shadow-2xl">
              ...
              <div className="space-y-3 mb-6">
                ...
                {newPwd && confirmPwd && newPwd === confirmPwd && (
                  <p className="text-emerald-600 text-[11px] font-bold text-center">
                    ✅ 비밀번호가 일치합니다.
                  </p>
                )}
              </div>
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 py-3.5 bg-slate-100 text-slate-600 font-bold rounded-xl active:bg-slate-200"
                >
                  취소
                </button>
                <button
                  onClick={handleChangePassword}
                  className="flex-1 py-3.5 bg-blue-600 text-white font-bold rounded-xl shadow-lg active:scale-95 transition-transform disabled:opacity-50"
                  disabled={newPwd !== confirmPwd || newPwd.length < 8}
                >
                  변경하기
                </button>
              </div>
        ...
        <div className="p-4 flex-1 overflow-y-auto flex flex-col">
          ...
          {receivableSummary.count > 0 && (
            <div className="mb-6 bg-red-50 border border-red-100 rounded-2xl p-4 shadow-sm">
              <button
                type="button"
                onClick={() => setShowReceivableDetails((prev) => !prev)}
              ...
              >
                <div>
                  ...
                  <p className="text-2xl font-extrabold text-red-600">
                    {receivableSummary.totalAmount.toLocaleString("ko-KR")}원
                  </p>
                  <p className="text-[11px] font-bold text-red-400 mt-1">
                    클릭하면 소비자별 미수금을 확인할 수 있습니다.
                  </p>
                </div>
                <div className="flex flex-col items-end gap-2 shrink-0">
                  <span className="bg-white text-red-600 border border-red-100 text-xs font-extrabold px-3 py-2 rounded-full">
                    {receivableSummary.count}건
                  </span>
                  <span className="text-red-500 text-lg font-extrabold">
                    {showReceivableDetails ? "접기 ▲" : "보기 ▼"}
                  </span>
                </div>
              </button>
              ...
              {showReceivableDetails && (
                <div className="mt-4 space-y-3 max-h-80 overflow-y-auto pr-1">
                  {receivableGroups.map((group: any) => (
                    <div
                    ...
                    >
                      ...
                      <div className="space-y-2">
                        {group.items.map((item: any) => (
                          <div
                            key={item.id}
                            className="bg-red-50/70 rounded-xl px-3 py-2 border border-red-100 flex items-center justify-between gap-2"
                          >
                            <div className="min-w-0">
                            ...
                            <div className="text-right shrink-0">
                              <p className="text-xs font-extrabold text-red-600 mb-1">
                              ...
                              <button
                                ...
                                className="bg-blue-600 text-white text-[10px] font-extrabold px-3 py-1.5 rounded-full shadow-sm active:scale-95 transition-transform"
                              >
                                입금완료 처리
                              </button>
                            </div>
          ...
          <div className="mb-6 space-y-3 shrink-0">
            <button
              ...
              className="w-full bg-slate-800 text-white py-4 rounded-2xl font-bold shadow-lg active:scale-95 transition-transform text-lg"
            >
              + 새 소비자 등록
            </button>
            <div className="flex gap-2">
              ...
              >
                ...
                <option value="concern">주요 고민</option>
                <option value="product">제품</option>
                <option value="consultation">상담 내용</option>
              </select>
              <input
                type="text"
                placeholder="검색어 입력"
            ...
            {filteredCustomers.length === 0 ? (
              ...
              filteredCustomers.map((c) => (
                <div
                  ...
                  className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex flex-col gap-3 active:bg-blue-50 cursor-pointer"
                >
                  <div className="flex justify-between items-start">
                    ...
                    >
                      <a
                        href={`tel:${c.phone}`}
                        className="w-9 h-9 flex items-center justify-center bg-green-50 text-green-600 rounded-full border border-green-100 shadow-sm active:bg-green-100 text-lg"
                      >
                        📞
                      </a>
                      <a
                        href={`sms:${c.phone}`}
                        className="w-9 h-9 flex items-center justify-center bg-blue-50 text-blue-600 rounded-full border border-blue-100 shadow-sm active:bg-blue-100 text-lg"
                      >
                        ✉️
                      </a>
                    </div>
                  </div>
                  {((c.concerns || []).length > 0 || c.concern_other) && (
                    <div className="flex flex-wrap gap-1.5">
                      {(c.concerns || []).map((concern: string) => (
                        <span
                          key={concern}
                          className="px-2 py-1 bg-blue-50 text-blue-700 border border-blue-100 rounded-full text-[10px] font-extrabold"
                        >
                          {concern}
                        </span>
                      ))}
                      {c.concern_other && (
                        <span className="px-2 py-1 bg-slate-50 text-slate-600 border border-slate-100 rounded-full text-[10px] font-extrabold">
                          기타: {c.concern_other}
                        </span>
                      )}
                    </div>
                  ...
                  <div className="flex justify-end gap-1 border-t border-slate-50 pt-2 mt-1">
                    <button
                      onClick={(e) => openEditCustomer(e, c)}
                      className="px-3 py-1.5 text-slate-400 hover:text-blue-600 bg-slate-50 rounded-lg text-xs font-bold"
                    >
                      정보 수정
                    </button>
                    <button
                      onClick={(e) => handleDeleteCustomer(e, c.id)}
                      className="px-3 py-1.5 text-slate-400 hover:text-red-500 bg-slate-50 rounded-lg text-xs font-bold"
                    >
                      삭제
                    </button>
                  </div>
          ...
          <div className="mt-8 pt-6 pb-4 border-t border-slate-200 text-center shrink-0">
            <button
              ...
              className="text-[11px] text-slate-400 font-medium underline underline-offset-2 hover:text-slate-600"
            >
              계정 탈퇴하기
            </button>
          </div>
      ...
      {isNotificationModalOpen && (
        <div className="fixed inset-0 bg-black/60 flex items-end sm:items-center justify-center p-4 z-[115]">
          <div className="bg-white w-full max-w-md max-h-[calc(100dvh-2rem)] overflow-y-auto rounded-[2rem] sm:rounded-[2.5rem] p-6 pt-8 pb-[calc(2rem+env(safe-area-inset-bottom))] shadow-2xl relative">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-slate-900">알림 설정</h2>
              <button
                onClick={() => setIsNotificationModalOpen(false)}
                className="px-3 py-1.5 bg-slate-200 rounded-full text-sm font-bold text-slate-800 active:bg-slate-300"
              >
                닫기
              </button>
            </div>
            ...
            <div className="space-y-3">
              {[
                ...
                <label
                  key={item.key}
                  className="flex items-center justify-between bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 cursor-pointer active:bg-blue-50"
                >
            ...
            <div className="space-y-3">
              <label className="flex items-center justify-between bg-blue-50 border border-blue-100 rounded-2xl px-4 py-3 cursor-pointer active:bg-blue-100">
                ...
                <input
                  ...
                  onChange={() =>
                    toggleNotificationSetting("daily_push_enabled")
                  }
                  className="w-5 h-5 accent-blue-600"
                />
              </label>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-6 z-[120]">
          <div className="bg-white w-full max-w-xs rounded-3xl p-6 shadow-2xl text-center">
            ...
            <button
              ...
              className="w-full py-3.5 bg-blue-600 text-white font-bold rounded-xl shadow-lg active:scale-95 transition-transform"
            >
              확인
            </button>
          </div>
          ...
          <div className="bg-white w-full max-w-xs rounded-3xl p-6 shadow-2xl text-center">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-3.5 bg-red-500 text-white font-bold rounded-xl shadow-lg active:scale-95 transition-transform"
              >
        ...
        <div className="fixed inset-0 bg-black/70 flex items-end sm:items-center justify-center p-0 sm:p-4 z-[60]">
          <div className="bg-white w-full max-w-md max-h-[85vh] overflow-y-auto rounded-t-[2rem] sm:rounded-[2.5rem] p-6 shadow-2xl relative flex flex-col">
            <div className="flex justify-between items-center mb-4 shrink-0">
              ...
              <button
                onClick={() => setShowCrmPopup(false)}
                className="px-4 py-2 bg-slate-200 rounded-full text-sm font-bold text-slate-800 active:bg-slate-300"
              >
                닫기
              </button>
            </div>
            <div className="space-y-3 overflow-y-auto pb-6">
              {crmAlerts.map((c) => (
                <div
                  ...
                  className={`${c.priority >= 5 ? "bg-orange-50 border-orange-100" : c.isSystem ? "bg-blue-50 border-blue-100" : "bg-red-50 border-red-100"} p-4 rounded-2xl border flex flex-col gap-3 cursor-pointer`}
                >
                  <div className="flex justify-between items-center">
                    <h3
                      className={`font-bold text-lg ${c.priority >= 5 ? "text-orange-900" : c.isSystem ? "text-blue-900" : "text-red-900"}`}
                    >
                      {c.name}
                    </h3>
                    {!c.isSystem && (
                      <button className="px-3 py-1.5 bg-white rounded-xl text-xs font-bold shadow-sm border text-red-600 border-red-100">
                        관리하기
                      </button>
                    )}
                  </div>
                  <div className="flex flex-col gap-1.5 w-full">
                    {c.reasons.map((r: string, idx: number) => (
                      <span
                        key={idx}
                        className={`text-xs font-medium bg-white px-2.5 py-1.5 rounded-lg shadow-sm border ${c.isSystem ? "text-blue-700 border-blue-100" : "text-red-700 border-red-100"}`}
                      >
                        {r}
                      </span>
                    ))}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/60 flex items-end sm:items-center justify-center p-0 sm:p-4 z-50">
          <div className="bg-white w-full max-w-md max-h-[92vh] overflow-y-auto rounded-t-[2rem] sm:rounded-[2.5rem] p-6 pt-8 pb-10 shadow-2xl relative">
            <h2 className="text-2xl font-bold text-slate-900 mb-6">
              {editingCustomerId ? "정보 수정" : "새 소비자 등록"}
            </h2>
            <form onSubmit={handleCustomerSubmit} className="space-y-3">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        birthday: formatDateInput(e.target.value),
                      })
                    }
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {CONCERN_OPTIONS.map((option) => (
                    <label
                      key={option}
                      className={`flex items-center gap-2 px-3 py-2.5 rounded-xl border text-sm font-bold cursor-pointer active:scale-[0.99] ${formData.concerns.includes(option) ? "bg-blue-50 border-blue-200 text-blue-700" : "bg-white border-slate-200 text-slate-700"}`}
                    >
                      <input
                        type="checkbox"
                        checked={formData.concerns.includes(option)}
                        onChange={() => toggleCustomerConcern(option)}
              ...
              <textarea
                placeholder="메모(소비자 특징)"
                className="w-full px-4 py-4 bg-white border border-slate-200 rounded-xl h-24 resize-none outline-none text-slate-900 font-medium"
                value={formData.memo}
                onChange={(e) =>
              ...
              <div className="flex gap-3 pt-2">
                <button
                  ...
                  className="flex-1 py-4 bg-slate-200 text-slate-800 font-bold rounded-xl"
                >
                  취소
                </button>
                <button
      ...
      {selectedCustomer && (
        <div className="fixed inset-0 bg-black/70 flex items-end sm:items-center justify-center p-0 sm:p-4 z-40">
          <div
            ref={modalScrollRef}
            className="bg-white w-full max-w-md h-[92vh] sm:h-[800px] overflow-y-auto rounded-t-[2rem] sm:rounded-[2.5rem] p-5 shadow-2xl relative scroll-smooth flex flex-col"
          >
            <div className="sticky top-0 z-20 bg-white -mx-5 px-5 -mt-5 pt-5 pb-3 border-b border-slate-100 shrink-0">
                  ...
                  <div className="flex items-center gap-3 mt-1.5">
                    ...
                    <div className="flex gap-1.5">
                      <a
                        ...
                        className="w-7 h-7 flex items-center justify-center bg-green-50 text-green-600 rounded-full border border-green-100 text-sm"
                      >
                        📞
                      </a>
                      <a
                        href={`sms:${selectedCustomer.phone}`}
                        className="w-7 h-7 flex items-center justify-center bg-blue-50 text-blue-600 rounded-full border border-blue-100 shadow-sm text-sm"
                      >
                        ✉️
                      </a>
                    </div>
              ...
              <div className="space-y-2 mt-1">
                ...
                {((selectedCustomer.concerns || []).length > 0 ||
                  selectedCustomer.concern_other) && (
                  <div className="flex flex-wrap gap-1.5">
                    {(selectedCustomer.concerns || []).map(
                      (concern: string) => (
                        <span
                          key={concern}
                          className="px-2 py-1 bg-blue-50 text-blue-700 border border-blue-100 rounded-full text-[10px] font-extrabold"
                    ...
                    {selectedCustomer.concern_other && (
                      <span className="px-2 py-1 bg-slate-50 text-slate-600 border border-slate-100 rounded-full text-[10px] font-extrabold">
                        기타: {selectedCustomer.concern_other}
                      </span>
                    )}
                  </div>
                )}
                {selectedCustomer.memo && (
                  <div className="bg-blue-50/50 border border-blue-100/50 rounded-2xl p-3 shadow-sm">
                    <p className="text-[13px] text-blue-800 leading-relaxed">
                      <span className="mr-1">📝</span> {selectedCustomer.memo}
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="mt-4 mb-6 shrink-0 space-y-3">
              <div className="bg-blue-50 p-4 rounded-2xl border border-blue-100 space-y-3">
                <h3 className="font-bold text-blue-900 text-sm">
                  {editingConsultationId ? "✏️ 상담 수정" : "💬 상담 기록"}
                </h3>
                <div className="flex flex-col gap-2">
                  <div className="flex gap-2">
                    <div className="flex-1 space-y-1">
                      <label className="text-[10px] font-bold text-blue-600 ml-1">
                        상담일
                      </label>
                      <div className="relative">
                        <input
                          ...
                          onBlur={(e) => validateDateOnBlur(e.target.value, "상담일")}
                        />
                      </div>
                    </div>
                    <div className="flex-1 space-y-1">
                      ...
                      <div className="relative">
                        <input
                          ...
                          onChange={(e) =>
                            setConsultationData({
                              ...consultationData,
                              next_meeting_date: formatDateInput(e.target.value),
                            })
                          }
                          onBlur={(e) =>
                            validateDateOnBlur(e.target.value, "다음 미팅일")
                          }
                        />
                      </div>
                    </div>
                  ...
                  <textarea
                    placeholder="상담 내용 입력 (엔터는 줄바꿈)"
                    className="w-full px-3 py-3 bg-white border border-blue-200 rounded-xl text-sm outline-none text-slate-900 font-medium h-20 resize-none"
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                  ...
                  <button
                    ...
                    className="flex-1 py-3 bg-blue-600 text-white rounded-xl text-sm font-bold shadow-sm"
                  >
                    {editingConsultationId ? "수정 완료" : "등록하기"}
                  </button>
                </div>
              </div>
              {!editingPurchaseId && (
                          ...
                          {item.type === "consultation" ? (
                            <>
                              ...
                              {item.next_meeting_date && (
                                <p className="mt-3 text-[11px] font-bold text-blue-600 bg-blue-50/50 inline-block px-2.5 py-1.5 rounded-lg">
                                  🗓️ 다음 미팅:{" "}
                                  {new Date(
                                    item.next_meeting_date,
